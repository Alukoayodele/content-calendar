...
import dev.blackdev.contentcalendar.model.Status;
import dev.blackdev.contentcalendar.model.Type;
...
public class ContentJdbcTemplateRepository {
    ...
    public void createContent(String tittle, String desc, Status status, Type contentType, String url){
        String sql = "INSERT INTO Content (tittle, desc, status, content_type, date_created, url) VALUES (?, ?, ?, ?, NOW(), ?)";
        jdbcTemplate.update(sql, tittle, desc, status, contentType, url);
    }

    public void updateContent(int id, String title, String desc, Status status, Type contentType, String URL) {
        String sql = "UPDATE Content SET tittle=?, desc=?, status=?, content_type=?, date_updated=NOW(), url=? WHERE id=?";
